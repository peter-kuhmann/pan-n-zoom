...
if (!window.customElements.get(PanNZoomPresentWebComponentTag)) {
  class PanNZoom extends HTMLElement {
    ...
    createHeader() {
      if (!this.hideTitle || !this.hideBranding) {
        ...
        if (!this.hideBranding) {
          ...
          this.header.append(this.headerBranding);
        } else {
          // Center header elements if no branding is shown
        ...
        if (!this.hideBranding) {
          ...
          this.headerFiller.classList.add("filler");
          this.header.append(this.headerFiller);
        }
